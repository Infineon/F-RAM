...
* Version: 1.0
*
* Description: This is a code example for accessing the QSPI F-RAM using PSoC6 SMIF
*
* Related Document: CE224034.pdf
*
...
int main()
{
    ...
    uint8_t volatile * const Xip_buffer_8 = (uint8_t *) MEMBASE;    /* 8 Bit buffer in external memory */
    uint32_t volatile source_buffer[BUFFER_SIZE];                   /* 32 Bit Local buffer for data transfer */
    uint8_t volatile source_buffer_8[BUFFER_SIZE];                  /* 8 bit local buffer for data transfer */
    uint32_t fillVal;
...
******************************************************************/
    ...
    for(loopcount = 0; loopcount < BUFFER_SIZE; loopcount++)
    {
        if (loopcount % 21  == 0 && loopcount > 0)/* New line every 21 values */
        {
            printf("\n\r");
        }    
...
******************************************************************/    
    ...
    fillVal = 0x11111111; /*fillVal should be > 0x11111110 for UART formatting */
    
    /* Fill source buffer with values shifted to fill 32 bits */
    for(loopcount = 1; loopcount < BUFFER_SIZE_XIP + 1; loopcount++)
    {
        source_buffer[loopcount-1] = fillVal * loopcount;
    }     
    
    /* Transfer source buffer data to Xip buffer */
    for(loopcount = 0; loopcount < BUFFER_SIZE_XIP; loopcount++)
    {
        Xip_buffer_32[loopcount] =  source_buffer[loopcount];
    } 
        
    /* Print source buffer */
    printf("\r\n\r\n::32-Bit data to write in XIP mode::\r\n");
    for(loopcount = 0; loopcount < BUFFER_SIZE_XIP; loopcount++)
    {
        ...
        printf("%#" PRIx32 " ", source_buffer[loopcount]);
    }
    printf("\r\n");
    
    /* Print XIP buffer */
    printf("\n\r::32-Bit data in XIP after write::\n\r");
    for(loopcount = 0; loopcount < BUFFER_SIZE_XIP; loopcount++)
    {
        if (loopcount % 9 == 0 && loopcount > 0)
        {
            printf("\n\r");
        }          
        printf("%#" PRIx32 " ",  Xip_buffer_32[loopcount]);
    } 
    printf("\r\n");
    
    /* Multiply the XIP buffer */
    MultiplyXArray(Xip_buffer_32, 25);
    
    /* Print multiplied XIP buffer */
    printf("\r\n::External XIP buffer after multiply by 2::\r\n");
    for(loopcount = 0; loopcount < BUFFER_SIZE_XIP; loopcount++)
    {
        if (loopcount % 9 == 0 && loopcount > 0)
        {
...
******************************************************************/      
    ...
    /* Fill source buffer with values shifted to fill 32 bits */
    for(loopcount_8 = 0; loopcount_8 < BUFFER_SIZE_XIP; loopcount_8++)
    {
        source_buffer_8[loopcount_8] = loopcount_8;
        /* Transfer local data to xip buffer */         
        Xip_buffer_8[loopcount_8] = source_buffer_8[loopcount_8];        
    } 

    /* Print source buffer */
    printf("\r\n\r\n::8-Bit data to write in XIP mode::\r\n");
    for(loopcount_8 = 0; loopcount_8 < BUFFER_SIZE_XIP; loopcount_8++)
    {
        printf("0x%02X ", source_buffer_8[loopcount_8]);
    }
    printf("\r\n");
    
    /* Print XIP buffer */
    printf("\n\r::8-Bit data in XIP after write::\n\r");
    for(loopcount_8 = 0; loopcount_8 < BUFFER_SIZE_XIP; loopcount_8++)
    {
        printf("0x%02X ", Xip_buffer_8[loopcount_8]);
    } 
    printf("\r\n");    
    
    /* Multiply the XIP buffer */
    MultiplyXArray_8(Xip_buffer_8, 25);
    
    /* Print multiplied XIP buffer */
    printf("\r\n::External XIP buffer after multiply by 2::\r\n");
    for(loopcount = 0; loopcount < BUFFER_SIZE_XIP; loopcount++)
    {
        printf("0x%02X ", Xip_buffer_8[loopcount]);
    }      
